...
-- Função para corrigir leads específicos baseado no domínio do website_url
CREATE OR REPLACE FUNCTION public.fix_lead_client_associations()
RETURNS TABLE(
    total_leads integer,
...
-- Função para reportar leads órfãos (para debug)
CREATE OR REPLACE FUNCTION public.report_orphaned_leads()
RETURNS TABLE(
    lead_id uuid,
    lead_name text,
    lead_email text,
...
-- Executar a função para corrigir as associações
SELECT * FROM public.fix_lead_client_associations();
